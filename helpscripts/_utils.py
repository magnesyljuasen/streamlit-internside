...
def hour_to_week(hourly_array):
    weekly_array = []
    summed = 0
    for i in range(len(hourly_array)):
        verdi = hourly_array[i]
        if np.isnan(verdi):
            verdi = 0
        summed += verdi
        if (i + 1) % 168 == 0:
            weekly_array.append(int(summed))
            summed = 0
    return weekly_array
...
def month_to_hour(monthly_array, leap_year = False):
    if leap_year == True:
        hourly_array = np.zeros(8760+24)
        n, m = 0, 744
        hourly_array[n:m] = monthly_array[0] / (m - n)
        n, m = 744, 1416 + 24
        hourly_array[n:m] = monthly_array[1] / (m - n)
        n, m = 1416 + 24, 2160 + 24
        hourly_array[n:m] = monthly_array[2] / (m - n)
        n, m = 2160 + 24, 2880 + 24
        hourly_array[n:m] = monthly_array[3] / (m - n)
        n, m = 2880 + 24, 3624 + 24
        hourly_array[n:m] = monthly_array[4] / (m - n)
        n, m = 3624 + 24, 4344 + 24
        hourly_array[n:m] = monthly_array[5] / (m - n)
        n, m = 4344 + 24, 5088 + 24
        hourly_array[n:m] = monthly_array[6] / (m - n)
        n, m = 5088 + 24, 5832 + 24
        hourly_array[n:m] = monthly_array[7] / (m - n)
        n, m = 5832 + 24, 6552 + 24
        hourly_array[n:m] = monthly_array[8] / (m - n)
        n, m = 6552 + 24, 7296 + 24
        hourly_array[n:m] = monthly_array[9] / (m - n)
        n, m = 7296 + 24, 8016 + 24
        hourly_array[n:m] = monthly_array[10] / (m - n)
        n, m = 8016 + 24, 8760 + 24
        hourly_array[n:m] = monthly_array[11] / (m - n)
...
class Plotting:
    ...
    def xy_simulation_plot(self, x, xmin, xmax, x_label, y1, y2, y_label, y_legend1, y_legend2, COLOR1, COLOR2, ymin = -10, ymax = 16):
        ...
        ax.axhline(y = 35, color = 'black', linestyle = '--', linewidth=0.3)
